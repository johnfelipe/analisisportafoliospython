...
# Set the page title
st.set_page_config(page_title='Aplicación de portafolio de acciones', page_icon=':chart_with_upwards_trend:')
...
st.title('📈 Aplicación de cartera de acciones')
st.write('Esta aplicación es una poderosa herramienta diseñada para ayudar a los usuarios '
         'a gestionar y realizar un seguimiento de sus portafolios de inversión. Con recuperación de datos bursátiles en tiempo real, '
         'los usuarios pueden agregar acciones a su portafolio, realizar un seguimiento de su desempeño y analizar métricas clave '
         'como el valor de la cartera, los rendimientos y más. La aplicación también ofrece funciones para comprar y vender '
         'acciones, actualizar los detalles de la cartera y comparar el rendimiento de la cartera con puntos de referencia populares.'
         ' Con una interfaz intuitiva y una funcionalidad integral, la aplicación permite a los usuarios tomar decisiones informadas '
         'de inversión y mantenerse al tanto de sus objetivos financieros.')
...
if get_session_state() is not None:
    ...
    st.header('🔍Recuperación de datos de mercado y cálculo de rendimiento logarítmico')
    st.markdown("<hr>", unsafe_allow_html=True)
    # Get user input for stock ticker symbol
    ticker = st.text_input("Ingrese el símbolo bursátil (por ejemplo, AAPL):")
    ...
    selected_timeframe = st.selectbox("Seleccione un período de tiempo:", list(timeframes.keys()))
    ...
    purchase_price = st.number_input('Precio de compra', min_value=0.0, step=0.01)
    purchase_date = st.date_input('Fecha de compra')
    add_stock_button = st.button('Añadir acciones')
    if add_stock_button and ticker and shares and purchase_price and purchase_date:
        ...
        if result:
            ...
            st.success('¡Acciones agregadas al portafolio!')
        else:
    ...
    st.header('📊 Actualizar una acción en su portafolio')
    st.markdown("<hr>", unsafe_allow_html=True)
    ...
    if portfolio_data:
        portfolio_dropdown_options = [f'{row[1]} ({row[2]})' for row in portfolio_data]
        portfolio_dropdown_index = st.selectbox('Seleccione una acción', range(len(portfolio_data)),
                                                format_func=lambda i: portfolio_dropdown_options[i])
        ...
        st.write(f"Acciones actuales: {current_shares}")
        st.write(f"Precio de compra: {current_purchase_price}")

        new_shares = st.number_input('Número de acciones', min_value=0, step=1, value=current_shares)

        if new_shares == current_shares:
            st.warning("Sin variación de acciones.")

        elif new_shares < current_shares:
            # Selling shares
            shares_sold = current_shares - new_shares
            selling_date = st.date_input('fecha de venta')

            if st.button('Vender acciones'):
                ...
                if stock_data.empty or stock_data['Close'].isnull().all():
                    st.error("No hay datos disponibles para la fecha y el símbolo bursátil especificados.")
                else:
            ...
            buying_price = st.number_input('Precio de compra', min_value=0.0, step=0.01, value=current_purchase_price)
            buying_date = st.date_input('Fecha de compra')
    ...
    st.header('🧾 Ver tus holdings de portafolio')
    portfolio_data = get_user_portfolio(st.session_state.user_id)
    if portfolio_data:
        # Create a DataFrame with the portfolio data
        df_portfolio = pd.DataFrame(portfolio_data,
                                    columns=['ID', 'Ticker', 'Compañia', 'Sector', 'Shares', 'Purchase Price',
        ...
        st.write(f'Rentabilidad total de la cartera: ${portfolio_value:.2f}')
    ...
    st.header('📝 Registros de transacciones')
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.header('💹 Compare el rendimiento de la cartera con un punto de referencia')
    st.markdown("<hr>", unsafe_allow_html=True)
    ...
    benchmark_dropdown_index = st.selectbox('Seleccione un punto de referencia', range(len(benchmark_data)),
    ...
    if start_date >= end_date:
        st.warning('Fecha de inicio debe ser anterior a la fecha de finalización.')
    else:
    ...
    st.header("Ingresa con tus credenciales para continuar")
